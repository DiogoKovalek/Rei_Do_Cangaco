...
public class GreenMan extends Entity {
    ...
    private BufferedImage[] ENEMY1_MOVE = {Game.spritesheet.getSprite(16 * 0, 16 * 2, 16, 16),
        Game.spritesheet.getSprite(16 * 1, 16 * 2, 16, 16)};
    private int frame = 0, maxFrame = 17, index = 0, maxIndex = 1;
    //private int frameMove = 0, maxFrameMove = Entity.rand.nextInt(60, 180), direction = Entity.rand.nextInt(2); //D=0 Horizontal D=1 Vertical
    private int frameDying = 0, maxFrameDying = 6, indexDying = 0, maxIndexDying = 5;

    private boolean died = false;
    private int life = 1;

    public GreenMan(double x, double y, int width, int height, double speed, BufferedImage sprite) {
...
//        if (!isColiddingWithPlayer() && !died) {
//            frameMove++;
//            if (frameMove == maxFrameMove) {
//                direction = Entity.rand.nextInt(2);
//                frameMove = 0;
//                maxFrameMove = Entity.rand.nextInt(60, 180);
//            }
//            if (direction == 0) {
//                if (this.x > Game.player.getX() && World.isFree((int) (this.x - speed), (int) this.y)) {
//                    this.x -= speed;
//                } else if (this.x < Game.player.getX() && World.isFree((int) (this.x + speed), (int) this.y)) {
//                    this.x += speed;
//                } else if (this.x == Game.player.getX()) {
//                    direction = 1;
//                }
//            } else if (direction == 1) {
//                if (this.y > Game.player.getY() && World.isFree((int) this.x, (int) (this.y - speed))) {
//                    this.y -= speed;
//                } else if (this.y < Game.player.getY() && World.isFree((int) this.x, (int) (this.y + speed))) {
//                    this.y += speed;
//                } else if (this.y == Game.player.getY()) {
//                    direction = 0;
//                }
//            }
//        } else if (!died) {//Damege to player
//
//        }
        if(!died){
